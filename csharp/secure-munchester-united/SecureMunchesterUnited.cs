public class SecurityPassMaker
{
    ...
    {
        if (support is not Security)
        {
            return (support is Staff) ? support.Title : "Too Important for a Security Pass";
        }
        else
        {
            return (support.GetType() == typeof(SecurityJunior) ||
            support.GetType() == typeof(SecurityIntern) ||
            support.GetType() == typeof(PoliceLiaison)) ? support.Title : $"{support.Title} Priority Personnel";
        }
    }
}
